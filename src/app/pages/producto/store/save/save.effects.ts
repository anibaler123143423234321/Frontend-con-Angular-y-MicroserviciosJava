...
export class SaveEffectsProducto {
  ...
  constructor(
    ...
    private generalService: GeneralService // Agrega GeneralService aquí
  ) { }
  ...
  read: Observable<Action> = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap(() => {
        // Obtén el negocioId desde GeneralService
        const negocioId = this.generalService.usuario$.negocioId;

        return this.httpClient.get<ProductoResponse[]>(`${environment.url}gateway/producto/pornegocio/${negocioId}`)
          .pipe(
            delay(1000),
